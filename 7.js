...
const package_namespaceObject = /*#__PURE__*/JSON.parse('{"UU":"lx-music-source","rE":"1.1.2","cy":"lxmusic"}');
...
const address = [
  ...
  `https://api.leobba.cn/${package_namespaceObject.cy}/${package_namespaceObject.UU}`,
...
const getVersion = async(index = 0) => {
  return getLatestVersion(address[index] + '/package.json').then(version => {
    return {
      version,
      url: address[index] + '/dist/xiaoxiongmao.js',
    }
  }).catch(async(err) => {
    index++
...
checkLatestVersion().then((version) => {
  if (!version) return
  lx_send(lx_EVENT_NAMES.updateAlert, { log: '发现新版本 v' + version.version + '请在公众号【大学资源社】中回复【音源】即可获取更新' })
})
